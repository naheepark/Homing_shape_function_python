...
#         if len(points) < 3: #! <------------------------------------------------------ 변수 조절
#             raise ValueError("최소 3개여야 합니다.")
#         X = []
#         Y = []
#         for x, y in points:
...
def circle_equation_function(a, b, r):
    def f(x, y):
...
def fit_circle_least_squares(points):
    """
    최소자승법을 이용하여 주어진 점들을 근사하는 원의 중심과 반지름을 구합니다.
    ...
    # 전개하면: x^2 + y^2 = 2ax + 2by + c
    A = np.c_[2*x, 2*y, np.ones(len(x))]     # 2x, 2y, 1로 구성된 행렬
    b = x**2 + y**2                          # 오른쪽 항: x^2 + y^2
    
    coeffs, *_ = np.linalg.lstsq(A, b, rcond=None)
    a, b, c = coeffs                         # 중심 좌표 (a, b), 상수항 c
    
    r = np.sqrt(c + a**2 + b**2)             # 반지름 계산

    return a, b, r


 # ----------------------------------------------------------------------------------------------   
save_folder = "remove_noise_" #! <--------------------------- 폴더명 설정
...
#* 타원 피팅 
    ...
    # 원 근사 강제 적용
    for _ in range(3):
        sample = random.sample(filtered_points, 3)
        a, b, r = fit_circle_least_squares(sample)
        ...
        if area < best_area:
            ...
            best_sample = sample
            best_coeffs = (a, b, r)
            best_func = circle_equation_function(a, b, r)
