...
def fit_ellipse_through_points(points): 
        ...
        if len(points) < 10: #! <------------------------------------------------------ 변수 조절
            raise ValueError("최소 10개여야 합니다.")
        X = []
        Y = []
        for x, y in points:
...
def cov_points_colinear(points, threshold=1e-2):
...
def remove_noise(points, threshold=2.5): #이상치 제거
    x = np.array([p[0] for p in points])
    y = np.array([p[1] for p in points])
...
#* 타원 피팅 
    ...
    if not cov_points_colinear(filtered_points):
        for _ in range(100):  # 샘플링 루프 시작
            sample = random.sample(filtered_points, 10)

            A, B, C, D, E, F = fit_ellipse_through_points(sample)
